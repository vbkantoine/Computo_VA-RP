...
alpha = 0.5
name_file = 'Probit_results_unconstrained_good.pkl'
...
plt.figure(figsize=(4, 3))
plt.plot(range_MI, target_w1(np.array(weights_hist['w1']).squeeze()), label=r'$\|w_1\|_2$', color='#f0a94c')
plt.plot(range_MI, target_b1(weights_hist['b1']), label=r'$|b_1|$')
plt.plot(range_MI, target_w2_1(np.array(weights_hist['w2']).squeeze()), label=r'$\max(w_2^{-1}<0)$', color='#8D725B')
plt.plot(range_MI, target_w2_2(np.array(weights_hist['w2']).squeeze()), label=r'$\min(w_2^{-1}>0)$', color='#7D1B65')
...
J = 500  #500   # nb samples for MC estimation in MI
...
T_cstr = 10000
eta_augm = torch.tensor([[0.,0.001]])
eta = torch.tensor([[0.,1.]])
name_file = 'Probit_results_constrained_new.pkl'
file_path = os.path.join(path_plot, name_file)

seed_all(0)
sqrt2 = np.sqrt(2)
act_bet = lambda x: torch.clamp(torch.log(1- 0.5*(1+torch.clamp(torch.erf(x/sqrt2), min=-1+1e-4, max=1-1e-4) ) ), min=-1e4)
NN_cst = NetProbitSeparate(input_size, m1=0, s1=0.1, b1=0, pre_act=[nn.Identity(), act_bet], act1=[torch.exp, torch.exp])

# NN_cst.load_state_dict(torch.load( os.path.join(path_plot, 'Probit_results_constrained_torch_state_good')))
...
with torch.no_grad():
    ...
    # NN_cst.netalpaha.singl.fc1.weight *= 3
...
if not load_results_Probit_cstr :
    ...
    MI, constr_values, range_MI, lower_MI, upper_MI = Constr.Partial_autograd(J, N, eta, num_epochs, optimizer, num_samples_MI, 
                                                            ...
                                                            num_samples_grad=40,
                                                            momentum=True,
                                                            freq_augm=freq_augm, max_violation=0.15, update_eta_augm=2)
    ...
    # Saves all relevant quantities
    Mutual_Info = {'values' : MI, 'range' : range_MI, 'lower' : lower_MI, 'upper' : upper_MI, 'constr' : constr_values, 'weights':Constr.keep_params}
    Prior = {'samples' : theta_sample, 'jeffreys' : None, 'params' : all_params}
    Posterior = {'samples' : theta_post_cstr, 'jeffreys' : None}
    Probit_results_cstr = {'MI' : Mutual_Info, 'prior' : Prior, 'post' : Posterior}
    weights_hist = Constr.keep_params
    with open(file_path, 'wb') as file:
        pickle.dump(Probit_results_cstr, file)

    torch.save(NN_cst.state_dict(),   os.path.join(path_plot, 'Probit_results_constrained_torch_state_new'))
...
plt.figure(figsize=(5, 4))
plt.hist(theta_sample_init[:,1], density=True, bins="rice", color="red", label=r"Initial prior in $\beta$", alpha=0.4)
plt.hist(remove_nan_and_inf(theta_sample[:,1]), density=True, bins="rice", label=r"Fitted prior", alpha=0.4)
